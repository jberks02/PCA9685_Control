...
public: PCA9685(double servoAngleMin = 0.0f, double servoAngleMax = 180.0f, uint address = 108, uint sdaPin = 8, uint sclPin = 9) {
...
private: int reg_read(const uint8_t reg, uint8_t* buf, const uint8_t nbytes) {
...
private: int reg_write(const uint8_t reg, uint8_t* buf, const uint8_t nbytes) {
...
public: void servoSetAngle(double angle, uint servo) {
    ...
    if (servo > 15) throw "Invalid Servo Number Asked to Adjust";
...
int main() {
    ...
    PCA9685 servoSet(0.0f, 180.0f, 64, 12, 13);
    ...
    while (true) {
        ...
        gpio_put(LED, 1);
        servoSet.servoSetAngle(0, 15);
        sleep_ms(500);
        gpio_put(LED, 0);
        servoSet.servoSetAngle(90, 15);
        sleep_ms(500);
        servoSet.servoSetAngle(180, 15);
        sleep_ms(500);
    }
